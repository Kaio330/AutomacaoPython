...
class NotaFiscalVivo(NotaFiscal):
    ...
    def adicionarValores(cls, pdf_texto):
        ...
        regex_emissao = r'(?i)(?:SÉRIE \d+ \/ DATA DE EMISSÃO:|DATA DE EMISSÃO:\s*|Data de Emissão:|Data de[^:]*?E|\BAIRRO\s*[:]?\s*\w+\s*\w+?\s*\w+?|BAIRRO\s*\w+[:]\w+\s*\(\w+\s*\w+\)|BAIRRO\s*\w+[:]\w+[.]?\s*\w+\s*\w+?\s*\w+?\s*\w+?\s*\w+?)\s*(\d{2}/\d{2}/\d{4}|(\d+)[^:]*:/?\s*(\d{2}/\d{4}))'
